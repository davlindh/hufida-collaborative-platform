    ...
    <ScrollArea className="h-screen">
      <div className="min-h-screen bg-white text-deepGreen-800 p-4 sm:p-8">
        <div className="container mx-auto max-w-5xl">
          <motion.h1 
            className="text-4xl font-bold mb-8 text-center text-deepGreen-700"
            {...fadeIn}
          >
            About HUFIDA
          </motion.h1>
          <motion.p 
            className="mb-12 text-lg text-center text-deepGreen-600"
            {...fadeIn}
              ...
              <TabsList className={`${neuCardStyles({ elevation: "low" })} bg-deepGreen-50 p-2 rounded-t-lg flex justify-center`}>
                {sections.map(section => (
                  <TabsTrigger 
                    key={section.id} 
                    value={section.id} 
                    className={`${neuButtonStyles({ variant: "secondary", size: "sm" })} text-sm flex items-center justify-center p-2 m-1 text-deepGreen-700`}
                  >
                    {section.icon}
